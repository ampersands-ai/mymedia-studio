...
const CustomCreation = () => {
  ...
  // Stuck-state detector: Auto-recover if UI thinks we're generating but database shows no active generations
  useEffect(() => {
    if (!state.localGenerating && !state.pollingGenerationId) return;

    const checkForStuckState = async () => {
      try {
        const { data: activeGenerations } = await supabase
          .from('generations')
          .select('id, status')
          .eq('user_id', user?.id)
        ...
        // If UI thinks we're generating but database shows no active generations
        if (!activeGenerations || activeGenerations.length === 0) {
          console.warn('⚠️ Stuck state detected: UI generating but no active generations in DB');
          
          // Check if the pollingGenerationId actually completed
          if (state.pollingGenerationId) {
            const { data: gen } = await supabase
              .from('generations')
              .select('id, status, is_batch_output, storage_path, type')
              .eq('id', state.pollingGenerationId)
              .single();

            if (gen?.status === 'completed') {
              console.log('🔄 Stuck state recovery: Found completed generation, fetching outputs...');
              
              let outputs: GenerationOutput[] = [];

              // Handle batch outputs
              if (gen.is_batch_output) {
                const { data: children } = await supabase
                  .from('generations')
                  .select('id, storage_path, output_index, model_id, type, provider_task_id')
                  .eq('parent_generation_id', state.pollingGenerationId)
              ...
              if (outputs.length > 0) {
                console.log('✅ Recovered outputs from stuck state');
                updateState({
                  generatedOutputs: outputs,
                  generatedOutput: outputs[0]?.storage_path || null,
                  selectedOutputIndex: 0,
          ...
          // No valid generation found - clear the stuck state
          console.log('🧹 Clearing stuck generating state');
          updateState({
            localGenerating: false,
            pollingGenerationId: null,
          });
          toast.info('Generation status synced');
        }
      } catch (error) {
    ...
    // Check every 15 seconds if we're in a generating state
    const interval = setInterval(checkForStuckState, 15000);
    
    // Also check immediately after 5 seconds
    const immediateCheck = setTimeout(checkForStuckState, 5000);

    return () => {
      clearInterval(interval);
      clearTimeout(immediateCheck);
    };
  }, [state.localGenerating, state.pollingGenerationId, user?.id, updateState, state.generateCaption]);
