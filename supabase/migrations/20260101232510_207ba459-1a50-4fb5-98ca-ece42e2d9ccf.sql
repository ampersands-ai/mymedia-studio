-- Recreate user_content_history view with timing columns
...
UNION ALL
 SELECT v.id,
    ...
    NULL::integer AS setup_duration_ms,
    NULL::integer AS api_duration_ms
   FROM video_jobs v
...
UNION ALL
 SELECT ve.id,
    ...
    NULL::text AS storage_path,
        CASE
            WHEN ve.status = 'done'::text THEN 'completed'::text
            WHEN ve.status = ANY (ARRAY['queued'::text, 'rendering'::text]) THEN 'processing'::text
            ELSE ve.status
        END AS status,
    COALESCE(ve.cost_credits, 0::numeric)::integer AS tokens_used,
    ve.created_at,
    NULL::timestamp with time zone AS caption_generated_at,
    NULL::jsonb AS provider_response,
    NULL::uuid AS parent_generation_id,
