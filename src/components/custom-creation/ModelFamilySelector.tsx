...
export const ModelFamilySelector: React.FC<ModelFamilySelectorProps> = ({
  ...
  return (
    <div className="space-y-2">
      <Label>Model</Label>
      <DropdownMenu modal={true} open={isDropdownOpen} onOpenChange={(open) => {
        ...
        >
          {Object.keys(modelsByFamily).sort((a, b) => {
            ...
            if (isSingleModel) {
              ...
              // Multi-model family - controlled submenu
              return (
                <DropdownMenuSub 
                  key={family}
                  open={openSubMenu === family}
                  onOpenChange={(open) => setOpenSubMenu(open ? family : null)}
                >
