...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (insertError) {
      logger.error('Failed to insert error log', insertError instanceof Error ? insertError : new Error(String(insertError) || 'Database error'), {
        userId: user?.id,
    ...
    logger.critical('Error in log-error function', err);
