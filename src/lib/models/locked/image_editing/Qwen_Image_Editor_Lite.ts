 ...
 * - Uses referenceImages for input image
...
export const SCHEMA = Object.freeze({
  ...
  properties: {
    ...
    negativePrompt: {
      ...
      description: "Describe what to avoid in the output",
      maxLength: 2000,
      showToUser: false,
      isAdvanced: true,
    },
    ...
    scheduler: {
      ...
      default: "UniPC",
      enum: SCHEDULERS,
      description: "Sampling algorithm - affects quality and style",
      showToUser: false,
      isAdvanced: true,
    },
    numberResults: {
  ...
  "x-order": ["positivePrompt", "referenceImages", "aspectRatio", "negativePrompt"],
});
...
export function preparePayload(inputs: Record<string, unknown>): Record<string, unknown> {
  ...
  // Scheduler (required for I2I)
  task.scheduler = inputs.scheduler || "UniPC";
...
export async function execute(params: ExecuteGenerationParams): Promise<string> {
  const { prompt, modelParameters, userId, startPolling, uploadedImageUrls } = params;

  // Build inputs with referenceImages from uploaded image URLs
  const existing = (modelParameters.referenceImages ?? null) as unknown;
  const referenceImages = uploadedImageUrls?.length
