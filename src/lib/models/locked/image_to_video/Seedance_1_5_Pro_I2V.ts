 ...
 * - Image field: input_urls (ARRAY, 1-2 images)
...
export const MODEL_CONFIG = {
  ...
  maxImages: 2, // I2V - 1-2 images required
  ...
  // Lock system
  isLocked: true,
  lockedFilePath: "src/lib/models/locked/image_to_video/Seedance_1_5_Pro_I2V.ts",
} as const;
...
export const SCHEMA = Object.freeze({
  type: "object",
  required: ["prompt", "input_urls", "aspect_ratio", "duration"],
  ...
  properties: {
    ...
    input_urls: {
      ...
      description: "Upload 1-2 images. Accepted: JPEG, PNG, WebP. Max 10MB each.",
      renderer: "image",
  ...
  "x-order": ["prompt", "input_urls", "aspect_ratio", "duration", "resolution", "generate_audio"],
});
...
export function validate(inputs: Record<string, unknown>): { valid: boolean; error?: string } {
  ...
  if (inputs.input_urls.length > 2) {
    return { valid: false, error: "Maximum 2 reference images allowed" };
  }
...
export async function execute(params: ExecuteGenerationParams): Promise<string> {
  ...
  // Upload images - required for I2V
