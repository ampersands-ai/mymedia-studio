      ...
      <Card className="border-border bg-muted/50">
        <CardContent className="p-4 space-y-4">
          {failedGenerationError ? (
            ...
            <>
          {/* Background processing banner - shows when polling stopped but generation continues */}
          {generationState.isBackgroundProcessing && !generationState.generatedOutput && generationState.generatedOutputs.length === 0 && (
            <div className="rounded-lg border border-blue-500/30 bg-blue-500/10 p-4">
              <div className="flex items-start gap-3">
                <Clock className="h-5 w-5 text-blue-500 mt-0.5 flex-shrink-0" />
                <div className="space-y-1">
                  <h3 className="font-semibold text-foreground">Generation In Progress</h3>
                  <p className="text-sm text-muted-foreground">
                    This generation is taking longer than usual. It will continue processing in the background.
                  </p>
                  <p className="text-sm text-muted-foreground">
                    You'll receive an email when it's complete, and you can find it in <strong>My Creations</strong>.
                  </p>
                </div>
              </div>
              <div className="flex gap-2 mt-4">
          ...
          {/* Show "navigate away" message when generation starts (not in background mode) */}
          {generationState.generationStartTime && !generationState.isBackgroundProcessing && !generationState.generatedOutput && generationState.generatedOutputs.length === 0 && (
