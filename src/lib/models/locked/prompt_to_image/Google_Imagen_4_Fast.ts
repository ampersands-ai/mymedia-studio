...
export const MODEL_CONFIG = {
  ...
  costMultipliers: { num_images: { "1": 1, "2": 2, "3": 3, "4": 4 } },
  ...
  // Lock system
  isLocked: true,
  lockedFilePath: "src/lib/models/locked/prompt_to_image/Google_Imagen_4_Fast.ts",
} as const;
...
export const SCHEMA = {
  ...
  required: ["prompt"],
  type: "object",
  "x-order": ["prompt", "num_images", "aspect_ratio", "negative_prompt", "seed"],
} as const;
...
export function calculateCost(inputs: Record<string, any>) {
  const numImages = String(inputs.num_images || "1") as keyof typeof MODEL_CONFIG.costMultipliers.num_images;
  return MODEL_CONFIG.baseCreditCost * (MODEL_CONFIG.costMultipliers.num_images[numImages] || 1);
}
