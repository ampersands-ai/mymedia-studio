/** Google Imagen 4 (prompt_to_image) - Record: 2d5f8c3a-9b7e-4f6d-8c1a-5e3b9a6f4d8c */
import { supabase } from "@/integrations/supabase/client";
import type { ExecuteGenerationParams } from "@/lib/generation/executeGeneration";

export const MODEL_CONFIG = { modelId: "google/imagen-4", recordId: "2d5f8c3a-9b7e-4f6d-8c1a-5e3b9a6f4d8c", modelName: "Google Imagen 4", provider: "kie_ai", contentType: "image", baseCreditCost: 3, estimatedTimeSeconds: 30, costMultipliers: {}, apiEndpoint: "/api/v1/jobs/createTask", payloadStructure: "wrapper", maxImages: 0, defaultOutputs: 1 } as const;

export const SCHEMA = { properties: { aspect_ratio: { default: "1:1", enum: ["1:1", "3:4", "4:3", "9:16", "16:9"], type: "string" }, number_of_images: { default: 1, maximum: 4, minimum: 1, type: "integer" }, output_format: { default: "jpeg", enum: ["jpeg", "png"], type: "string" }, prompt: { maxLength: 5000, renderer: "prompt", type: "string" }, seed: { type: "integer" } }, required: ["prompt"], type: "object" } as const;

export function validate(inputs: Record<string, any>) { return inputs.prompt ? { valid: true } : { valid: false, error: "Prompt required" }; }
export function preparePayload(inputs: Record<string, any>) { return { modelId: MODEL_CONFIG.modelId, input: { prompt: inputs.prompt, aspect_ratio: inputs.aspect_ratio || "1:1", number_of_images: inputs.number_of_images || 1, output_format: inputs.output_format || "jpeg", ...(inputs.seed && { seed: inputs.seed }) } }; }
