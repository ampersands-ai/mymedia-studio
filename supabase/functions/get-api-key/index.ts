...
function getKieApiKey(modelId: string, recordId: string, contentType?: string): string {
  const veo3Models = [
  ...
  // Check specific models first (by recordId)
  ...
  else if (contentType === 'image_editing') secretName = 'KIE_AI_API_KEY_IMAGE_EDITING';
  else if (contentType === 'image_to_video') secretName = 'KIE_AI_API_KEY_IMAGE_TO_VIDEO';
  else if (contentType === 'prompt_to_image') secretName = 'KIE_AI_API_KEY_PROMPT_TO_IMAGE';
  else if (contentType === 'prompt_to_video') secretName = 'KIE_AI_API_KEY_PROMPT_TO_VIDEO';
  else if (contentType === 'prompt_to_audio') secretName = 'KIE_AI_API_KEY_PROMPT_TO_AUDIO';
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { provider, modelId, recordId, contentType } = await req.json();
    ...
    if (provider === 'kie_ai') {
      ...
      apiKey = getKieApiKey(modelId, recordId, contentType);
