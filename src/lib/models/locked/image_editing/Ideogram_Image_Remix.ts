...
export const MODEL_CONFIG = { modelId: "ideogram/image-remix", recordId: "fa2d60c2-4fc5-4d77-bc60-36e10dbf9e2b", modelName: "Ideogram Image Remix", provider: "kie_ai", contentType: "image", baseCreditCost: 1.75, estimatedTimeSeconds: 25, costMultipliers: { num_images: { 1:1, 2:2, 3:3, 4:4 }, rendering_speed: { BALANCED:2, QUALITY:3, TURBO:1 } }, apiEndpoint: "/api/v1/jobs/createTask", payloadStructure: "wrapper", maxImages: 1, defaultOutputs: 1 } as const;
...
export function calculateCost(inputs: Record<string, any>) { return MODEL_CONFIG.baseCreditCost * (MODEL_CONFIG.costMultipliers.num_images[parseInt(inputs.num_images || "1")] || 1) * (MODEL_CONFIG.costMultipliers.rendering_speed[inputs.rendering_speed || "TURBO"] || 1); }
