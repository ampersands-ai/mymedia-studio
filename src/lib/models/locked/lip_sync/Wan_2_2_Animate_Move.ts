...
export const MODEL_CONFIG = {
  ...
  modelFamily: "Wan",
  variantName: "2.2 Animate Move",
  displayOrderInFamily: 7,
  // Lock system
  isLocked: true,
  lockedFilePath: "src/lib/models/locked/lip_sync/Wan_2_2_Animate_Move.ts",
} as const;
...
export const SCHEMA = Object.freeze({
  ...
  properties: {
    video_url: {
      ...
      format: "uri",
      title: "Motion Video",
      description: "Video to extract motion from. Formats: MP4, MOV, MKV (max 10MB, 30s max)",
      renderer: "video",
    },
    image_url: {
      type: "string",
      format: "uri",
      title: "Target Image",
      description: "Image to animate with motion. Will be resized/cropped to match aspect ratio. Formats: JPEG, PNG, WebP (max 10MB)",
      renderer: "image",
    },
    resolution: {
      type: "string",
...
export function validate(inputs: Record<string, unknown>): { valid: boolean; error?: string } {
  ...
  if (!inputs.image_url || (typeof inputs.image_url === "string" && inputs.image_url.trim() === "")) {
    return { valid: false, error: "Target image is required" };
  }
...
export async function execute(params: ExecuteGenerationParams): Promise<string> {
  ...
  const description = "Animate Move";
