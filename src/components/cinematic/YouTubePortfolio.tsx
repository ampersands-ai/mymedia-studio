  ...
  return (
    <AnimatedSection delay={index * 100}>
      ...
      >
        ...
        <div className="absolute bottom-0 left-0 p-4 translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none flex flex-col items-start gap-1">
          <span className="text-xs font-medium uppercase tracking-wider text-primary-orange backdrop-blur-md bg-black/40 rounded px-2 py-0.5 border border-white/10">
            {item.category}
          </span>
          <h3 className="text-lg font-bold text-white backdrop-blur-md bg-black/40 rounded px-2 py-1 border border-white/10">{item.title}</h3>
        </div>
      </div>
    </AnimatedSection>
  );
