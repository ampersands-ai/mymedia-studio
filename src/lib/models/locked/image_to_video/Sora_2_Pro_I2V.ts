/** Sora 2 Pro Image-to-Video (image_to_video) - Record: c1d5e6f7-2a3b-4c5d-6e7f-8a9b0c1d2e3f */
...
export const MODEL_CONFIG = {
  ...
  modelFamily: "Sora",
  variantName: "2 Pro Image-to-Video",
  displayOrderInFamily: 5,
  // Lock system
  isLocked: true,
  lockedFilePath: "src/lib/models/locked/image_to_video/Sora_2_Pro_I2V.ts",
} as const;
...
export function preparePayload(inputs: Record<string, any>) {
  ...
  if (inputs.size) payload.size = inputs.size;
...
export function calculateCost(inputs: Record<string, any>) {
  ...
  const sizeKey = (inputs.size || "standard") as keyof typeof MODEL_CONFIG.costMultipliers.size;
  const sizeMult = MODEL_CONFIG.costMultipliers.size[sizeKey] || 1;
  return Math.round(base * framesMult * sizeMult * 100) / 100;
}
