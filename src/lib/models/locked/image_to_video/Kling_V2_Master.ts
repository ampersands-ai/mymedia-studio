...
export const MODEL_CONFIG = { modelId: "kling/v2-1-master-image-to-video", recordId: "c3397c13-3a52-4973-a87c-b4c20aca0fc0", modelName: "Kling V2 Master", provider: "kie_ai", contentType: "image_to_video",
  use_api_key: "KIE_AI_API_KEY_IMAGE_TO_VIDEO", baseCreditCost: 40, estimatedTimeSeconds: 300, costMultipliers: { duration: { 10: 2, 5: 1 } }, apiEndpoint: "/api/v1/jobs/createTask", payloadStructure: "wrapper", maxImages: 1, defaultOutputs: 1, 
  // UI metadata
  isActive: true,
...
export function calculateCost(inputs: Record<string, any>) { const durKey = parseInt(inputs.duration || "5") as keyof typeof MODEL_CONFIG.costMultipliers.duration; return MODEL_CONFIG.baseCreditCost * (MODEL_CONFIG.costMultipliers.duration[durKey] || 1); }
