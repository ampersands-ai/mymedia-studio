/**
 * Runware Seedance 1.0 Pro Fast (Image-to-Video)
 *
 ...
 * - Uses frameImages array with inputImage objects
...
export const MODEL_CONFIG = {
  ...
  maxImages: 1, // I2V - single input image
  defaultOutputs: 1,
  // Runware-specific
  ...
  variantName: "Seedance 1.0 Pro Fast (I2V)",
  displayOrderInFamily: 2,
  // Lock system
  isLocked: true,
  lockedFilePath: "src/lib/models/locked/image_to_video/Runware_Seedance_Pro_Fast_I2V.ts",
} as const;
...
export const SCHEMA = Object.freeze({
  ...
  properties: {
    inputImage: {
      type: "string",
      title: "Input Image",
      description: "Image URL or UUID to animate",
      renderer: "image",
    },
    positivePrompt: {
      type: "string",
      title: "Prompt",
      default: "",
      description: "Describe how the image should animate",
      maxLength: 2000,
      renderer: "prompt",
    },
    aspectRatio: {
  ...
  "x-order": ["inputImage", "positivePrompt", "aspectRatio", "duration"],
});
...
export function validate(inputs: Record<string, unknown>): { valid: boolean; error?: string } {
  // Input image is required for I2V
...
export function preparePayload(inputs: Record<string, unknown>): Record<string, unknown> {
  ...
  const task: Record<string, unknown> = {
    ...
    // frameImages array with inputImage objects
    frameImages: [
      {
