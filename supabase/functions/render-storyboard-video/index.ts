...
serve(async (req) => {
  ...
  try {
    ...
    // Determine which template to use based on content type (videos > images > prompts)
    ...
    // NEW: Check for video animations
    const scenesWithVideo = scenes.filter(s => !!s.video_url);
    ...
    if (allScenesHaveVideo) {
      // Priority 1: All scenes are animated videos
      templateId = '12dlFdikNAml1nfjKDaS';
      templateType = 'all-video';
      console.log('[render-storyboard-video] Using all-video template (animated scenes)');
    } else if (hasIntroPreview && allScenesHavePreview) {
      // Priority 2: All images are pre-generated (no animations)
      ...
      // Priority 4: Mixed (some previews, some prompts)
  ...
  - Scenes: ${scenesWithPreview.length}/${scenes.length} pre-generated`);
    ...
    // Add intro content based on priority: video > image > prompt
    ...
    // Build scenes array with appropriate content properties (priority: video > image > prompt)
    variables.scenes = scenes.map(s => {
      const sceneData: Record<string, any> = {
        voiceOverText: s.voice_over_text,
      };
      
      // Priority: video > image > prompt
      if (s.video_url) {
